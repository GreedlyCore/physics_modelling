...
h = 0.1
t = np.array([i * h * (2 * 10 ** -5) for i in range(1, int(100 / h) + 2)])  # время
# Frequency, Hz
f = 1 * 10**3
# omega = 2 * np.pi * f
...
def update_end_time(sender):
    ...
    t = [i * h * (2 * 10 ** -5) for i in range(1, int(100 / h) + 2)]  # время
    solver.set_t(t)

def update_C(sender):
    global C, solver
    C = round(dpg.get_value(sender),1)
    solver.set_C(C)
    dpg.set_value('cutoff_freq_tag', f"Cut off frequency: {1/(2*np.pi*r*C)} Hz")

def update_R(sender):
    global R, solver
    R = round(dpg.get_value(sender),1) 
    solver.set_R(R)
...
with dpg.window(label="Setup input signal and scheme", tag="in_signal"):
    ...
    slider_E0 = dpg.add_slider_float(label="Amplitude, V",
                                    ...
                                    min_value=0,
                                    callback=update_E0)
    slider_freq = dpg.add_input_float(label="f, kHz",
                                        format="%e",
                                        default_value=1400,
                                        callback=update_f)
    slider_time = dpg.add_input_float(label="end time, seconds",
                                      format="%e",
                                      default_value=3*10**-5,
                                      callback=update_end_time)
    slider_C = dpg.add_input_float(label="Capacity, farads",
                                    format="%e",
                                    default_value = 47 * 10 ** -5,
                                    callback=update_C)
    slider_r = dpg.add_input_float(label="Resistance - inner, Ohms",
                                    format="%e",
                                    default_value=8 * 10 ** 4,
                                    callback=update_r)
    slider_R = dpg.add_input_float(label="Resistance - outer, Ohms",
                                    format="%e",
                                    default_value=10 ** 5,
                                    callback=update_R)
